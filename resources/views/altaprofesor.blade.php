 ...
 <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        ...
                        <div class="row m-t-30">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">ALTA PROFEOSRES</div>
                                    <div class="card-body">
                                        <div class="card-title">
                                            <h3 class="text-center title-2">Ingresa Datos del Profesor</h3>
                                        </div>
                                        <hr>
<form action = "{{ route('guardaprofesor')}}" method = "POST">
{{csrf_field()}}
...
@endif	<p>
<table><tr><td>Clave Alumno</td><td style="width:100px"><input  class="form-control" type='text'  name='idprof' value="{{ $idprof}}" >
</td>
</td></tr></table>
<br>
...
@endif
<input  class="form-control"  placeholder="Ingrese Nombre " type='text'  name='nombre'  value="{{old ('nombre')}}" >
...
@endif	<input class="form-control" type='text' name='ap_emp' placeholder="Ingrese Apellido Paterno" value="{{ old('ap_emp')}}" >
...
@endif	<input class="form-control" type='text' name='am_emp' placeholder="Ingrese Apellido Materno" value="{{old ('am_emp')}}" >
...
@endif	
<input class="form-control" type='text' name='edad' placeholder="Ingrese Edad" value="{{  old('edad')}}" >
</p></td><td style="width:50px" align ="right">
    
        Sexo: </td><td> @if($errors->first('sexo'))
    <p class="text-danger"><i> {{ $errors->first('sexo') }} </i></p>
    @endif	<input class="form-control" type='text' name='sexo' placeholder="Ingrese Sexo" value="{{ old('sexo')}}" >
    ...
    <td> @if($errors->first('telefono'))
        ...
        @endif	
        <input class="form-control" type='text' name='telefono' placeholder="Ingrese Telefono" value="{{ old('telefono')}}" >
                ...
                <td style="width:160px" align ="right">
                    Correo Electronico:</td><td> @if($errors->first('correo'))
                        ...
                        @endif	
                        <input class="form-control" type='text' name='correo' placeholder="Ingrese Correo Electronico" value="{{old('correo')}}" >
                    </p></td><td style="width:70px" align ="right">
                    </tr>
                </td>
            </tr>
...
<table>
   <tr>
       <td>                                          
<table><tr><td style="width:60px" align ="right">
Calle</td><td> @if($errors->first('calle'))
    <p class="text-danger"><i> {{ $errors->first('calle') }} </i></p>
    @endif	<input class="form-control" type='text' name='calle' placeholder="Ingrese Calle" value="{{ old ('calle')}}" >
</p></td><td style="width:50px" align ="right">

            Colonia</td><td> @if($errors->first('colonia'))
            <p class="text-danger"><i> {{ $errors->first('colonia') }} </i></p>
            @endif	<input class="form-control" type='text' name='colonia' placeholder="Ingrese Colonia" value="{{ old('colonia')}}" >
            </p></td>
            <td style="width:50px" align ="right">
                    Número de casa:</td><td> @if($errors->first('num_calle'))
                            <p class="text-danger"><i> {{ $errors->first('num_calle') }} </i></p>
                            @endif	<input class="form-control" type='text' name='num_calle' placeholder="Número de Casa" value="{{ old('num_calle')}}" >
                        </p></td><td style="width:150px" align ="right">
                               
                    Código Postal:</td><td> @if($errors->first('cp'))
                        <p class="text-danger"><i> {{ $errors->first('cp') }} </i></p>
                        @endif	
                        <input class="form-control" type='text' name='cp' placeholder="Ingrese Código Postal" value="{{old ('cp')}}" >
                        </p></td>
